<template>
  <div>
    <div id="myChart21" style="height: 95%;width: 430px;position: absolute; margin: 10px 5px 5px 5px;"></div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    watch:{
      ...
      eData:{
        handler(val, oldVal){
          ...
          this.legendData = this.eData.echart21.legendData
          this.xAxisData = this.eData.echart21.xAxisData
          this.seriesData = this.eData.echart21.seriesData
          this.draw();
          this.myChart.resize();
        },
        deep:true
    ...
    mounted(){
      this.myChart = echarts.init(document.getElementById('myChart21'))
    },
    methods: {
      draw(){
